...
import './Search.css';
import { api_url, openweathermap_api_key } from "../../globalVariables.js";
...
const Search = ({ maxHeight }) => {
  ...
  useEffect(() => {
    ...
    const response = axios.get(`${api_url}${selectedCity}`).then((response) => {
      ...
      <Container>
        ...
        <Grid container>
          ...
          <Grid item md={6} sm={12} sx={{ pt: 6 }}>
            <Box sx={{ px: 8 }}>
              <img src={`https://linux-alex.github.io/weather-images/SVG/` + weatherIconsToImageNames[weather.average.icon]} alt={weather.average.weather} />
            </Box>
