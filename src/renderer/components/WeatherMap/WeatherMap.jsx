...
import { api_url, openweathermap_api_key } from '../../globalVariables';


const WeatherMap = ({ maxHeight }) => {
  const [weatherData, setWeatherData] = useState([]);
  const API_KEY =  openweathermap_api_key; //process.env.REACT_APP_OPENWEATHERMAP_API_KEY;
  ...
  useEffect(() => {
    window.electron.ipcRenderer.once('location', (arg) => {
      // navigator.geolocation.getCurrentPosition(
        // (position) => {
          const { latitude, longitude } = arg.geolocation;
  ...
  const getMarkerIcon = (data) => {
    return new L.Icon({
      ...
      iconUrl:  `https://linux-alex.github.io/weather-images/SVG/` /*`/weather-images/SVG/`*/ + weatherIconsToImageNames[data.weather[0].icon],
      iconSize: [72, 72],
    });
  };
